Ext.define('AM.controller.Animales', {
    extend: 'Ext.app.Controller',
   
    views:  ['AltaAnimal', 'ConsultaAnimal','CambioUbicacionAnimal','AltaEspecie','AltaPesoAnimal','DetalleAnimal','ModificarAnimal','CambioEstado','ActualizarImagenAnimal', 'AsignarPatrocinadorAnimal'],
    stores: ['Especies', 'Animales','HistoricoPesos','HistoricoUbicaciones','ComboSN','ComboHM','ComboTM','CriteriosBusquedaAnimal','ComboTI','ResultadoBusquedaAnimales','ResultadoBusquedaInstitucionPersona','CriteriosBusquedaPersonaInstitucion','EstadosAnimal'],
    models: ['Especie', 'Animal','Ubicacion','HistoricoPeso','HistoricoUbicacion','Combo','ResultadoBusquedaAnimales'],
    ...
    init: function() { 
	    ...
	    aceptarAsignarpatrocinador: function(button){	
	    	var seleccion = button.up('asignarPatrocinadorAnimal').query('gridpanel[name="ResBusquedaInstPersPatrocinador"]')[0].getSelectionModel().getLastSelected();
	    	if(seleccion.get('tipoResultado') == "Persona"){
	    		damePantallaAnterior().query('textfield[name="textoPatrocinador"]')[0].setValue(seleccion.get('nombrePersona') + " " + seleccion.get('apellidos'));
		    	damePantallaAnterior().query('hiddenfield[name="idPatrocinador"]')[0].setValue(seleccion.get('idPersona'));
	    	}else{
	    		damePantallaAnterior().query('textfield[name="textoPatrocinador"]')[0].setValue(seleccion.get('nombreInstitucion'));		    	
		    	damePantallaAnterior().query('hiddenfield[name="idPatrocinador"]')[0].setValue(seleccion.get('nombreInstitucion'));
	    	}
	    	damePantallaAnterior().query('textfield[name="txtMuestraMasInfo"]')[0].setValue(seleccion.get('info'));
	    	damePantallaAnterior().query('hiddenfield[name="tipoPatrocinador"]')[0].setValue(seleccion.get('tipoResultado'));
	    	vueltaPantalla();
		},
	    cerrarSinPatrocinador: function(button){			
	    	damePantallaAnterior().query('textfield[name="textoPatrocinador"]')[0].setValue('');
	    	damePantallaAnterior().query('textfield[name="txtMuestraMasInfo"]')[0].setValue('');
	    	damePantallaAnterior().query('hiddenfield[name="tipoPatrocinador"]')[0].setValue('');
	    	damePantallaAnterior().query('hiddenfield[name="idPatrocinador"]')[0].setValue('');
	    	
	    	vueltaPantalla();
		},
	    seleccionFilaPatrocinadores: function(grid, record){
	    	grid.up('asignarPatrocinadorAnimal').query('button[action="aceptar"]')[0].enable();
		},
	    buscarPatrocinador: function(txt){			
			...
			txt.up('asignarPatrocinadorAnimal').query('button[action="aceptar"]')[0].disable();
	    ...
	    cambioComboEspecie: function(combo, records, eOpts){
	    	...
	    	var txtRaza = combo.up('form').down('textfield[name=raza]');
	    	if(txtRaza.getValue() != ""){
		    	if(valor == "GATO"){
	    ...
	    masInfoAltaAnimal: function(button){			
			var txtMasInfo = button.up('form').query('textfield[name=txtMuestraMasInfo]')[0];
	    ...
	    actualizarImagen: function(button){			
			...
			form.submit({
		            ...
		            success: function(form, action) {
		               Ext.Msg.alert('Modificación realizada correctamente', action.result.msg);
		               vueltaPantalla();
		            },
		            failure: function(form, action) {
		                Ext.Msg.alert('Error en la modificación', action.result.msg);
		            }
		        });
	...
	cambiarPesoAnimal: function(button){
		...
		form.submit({
				waitMsg: 'En proceso...',
	            success: function(form, action) {
	               Ext.Msg.alert('Cambio peso realizado correctamente', action.result.msg);
	               vueltaPantallaRecarga('ResultadoBusquedaGeneral');
	            },
	            failure: function(form, action) {
	                Ext.Msg.alert('No se ha podido realizar el cambio', action.result.msg);
	            }
	        });
	...
	detalleAnimal: function(button){  
		...
		view.loadRecord(button.up('consultaAnimal').query('gridpanel')[0].getSelectionModel().getLastSelected());			
	...
	seleccionFilaBusqueda: function(grid, record){
		grid.up('cambioUbicacionAnimal').query('textfield[name="textoBusquedaUbicacion"]')[0].setValue('');
    	Ext.StoreManager.lookup('Ubicaciones').load({
			params: record.data,
			scope: this					
		});		
		
		grid.up('cambioUbicacionAnimal').query('button[action="cambioUbicacion"]')[0].disable();
	...
	seleccionFilaUbicaciones: function(grid, record){
		...
		grid.up('form').query('button[action="cambioUbicacion"]')[0].enable();
    ...
    cambioUbicacionAnimal: function(button){	
		var idAnimal = button.up('cambioUbicacionAnimal').query('hiddenfield[name="id"]')[0].getValue();
		var data = button.up('cambioUbicacionAnimal').query('gridpanel[name=Ubicacion]')[0].getSelectionModel().getLastSelected().data;
		...
		Ext.Ajax.request({
			...
			failure: function(form, action) {
				Ext.Msg.alert('No se ha podido realizar el cambio');
			}
		});
	},
	...
	mostrarHistoricoPesos: function(button){
    		var items = button.up('form').query('gridpanel[name="tablaPesos"]');
	...
	mostrarHistoricoUbicaciones: function(button){
    	var items = button.up('form').query('gridpanel[name="tablaUbicaciones"]');
		for (var i = 0, l = items.length; i < l; i++) {
			var componente = items[i];
		    if(componente.hidden == true){
				componente.show();
				var form = button.up('form').getForm();
				console.log( form.getValues());		
				Ext.StoreManager.lookup('HistoricoUbicaciones').load({
					params: form.getValues(),
					scope: this					
				});
			}else{
	...
	buscarInstitucionUbicacion: function(button){  
		...
		button.up('cambioUbicacionAnimal').query('textfield[name="textoBusquedaUbicacion"]')[0].setValue('');
		Ext.StoreManager.lookup('Ubicaciones').removeAll();
		Ext.StoreManager.lookup('AnimalesEnUbicacion').removeAll();
		button.up('cambioUbicacionAnimal').query('button[action="cambioUbicacion"]')[0].disable();
		
	},
	
	filtrarUbicacion: function(textfield){ 
		var selection = textfield.up('cambioUbicacionAnimal').query('gridpanel[name="ResultadoBusquedaInstitucionPersona"]')[0].getSelectionModel();
		if(!selection.hasSelection()){
			selection.select(0,false,true);
		}
		var record = textfield.up('cambioUbicacionAnimal').query('gridpanel[name="ResultadoBusquedaInstitucionPersona"]')[0].getSelectionModel().getLastSelected().data;
	...
	CambioCriterioSeleccion: function(newValue, oldValue){
		...
		var items = newValue.up('form').query('textfield,combo');
		for (var i = 0, l = items.length; i < l; i++) {
		    var cls = items[i].cls;
	...
	CambioCriterioSeleccionPI: function(newValue, oldValue){
		...
		form.query('fieldcontainer[name=instituciones]')[0].hide();
		form.query('fieldcontainer[name=personas]')[0].hide();
		form.query('button[action="cambioUbicacion"]')[0].disable();
	}
