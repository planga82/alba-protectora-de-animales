Ext.define('AM.controller.Listados', {
	...
	stores : [ 'comboListadoAtributos', 'ComboOpcionesListado',
			'ResultadoListados', 'ComboANDOR' ],
	models : [ 'Atributos' ],
	...
	busquedaListado : function(button) {
		...
		for (var i = 0; i < filtros.length; i++) {
			...
			filtro[txt.getFieldLabel() + "_String_" + contador++] = txt
					.getValue();
		}
		filtros = form.query('filtroBoolean');
		for (var i = 0; i < filtros.length; i++) {
			var txt = filtros[i].down('combo');
			filtro[txt.getFieldLabel() + "_Boolean_" + contador++] = txt
					.getValue();
		}
		...
		for (var i = 0; i < filtros.length; i++) {
			...
			filtro[lab.text + "_Date_" + contador + "_Desde"] = fld
					.getRawValue();
			filtro[lab.text + "_Date_" + contador++ + "_Hasta"] = flh
					.getRawValue();
		}
