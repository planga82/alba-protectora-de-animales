		...
		.define(
				'AM.controller.Principal',
				{
					...
					detalleAnimalclick : function(button) {
						...
						var record = button.up('panelPrincipal2').query(
						...
						if (imagen != null) {
							view.query('image')[0]
									.setSrc('images/imagenesTrabajo/' + imagen);
						}
						irPantalla('panelPrincipal2', view);
					...
					modificarAnimalclick : function(button) {
						...
						view.loadRecord(button.up('panelPrincipal2').query(
								'gridpanel')[0].getSelectionModel()
								.getLastSelected());
						irPantalla('panelPrincipal2', view);
					},
					...
					ubicacionesInstitucionclick : function(button) {
						...
						Ext.StoreManager.lookup('Ubicaciones').load(
								{
									params : button.up('panelPrincipal2')
											.query('gridpanel')[0]
											.getSelectionModel()
						...
						items = view
								.query('gridpanel[name="AnimalesUbicacion"]');
					...
					detalleInstitucionclick : function(button) {
						...
						view.title = 'Detalle Instituci�n';
						...
						var estado = button.up('panelPrincipal2').query(
								'gridpanel')[0].getSelectionModel()
								.getLastSelected().getData().estado;
						if (estado == 'BAJA') {
					...
					seleccionFilaBusquedaGeneral : function(grid, record) {
						...
						if (resultado == 'Persona') {
							grid.up('panelPrincipal2').query(
									'panel[name="operacionesPersona"]')[0]
									.show();
							grid.up('panelPrincipal2').query(
									'panel[name="operacionesInstitucion"]')[0]
									.hide();
							grid.up('panelPrincipal2').query(
									'panel[name="operacionesAnimal"]')[0]
									.hide();
							var panel = grid.up('panelPrincipal2');
							var estado = record.getData().estado;
							if (estado == 'BAJA') {
								panel
										.query('button[action="modificarPersona"]')[0]
										.setDisabled(true);
								panel
										.query('button[action="ubicacionesPersona"]')[0]
										.setDisabled(true);
							} else {
								panel
										.query('button[action="modificarPersona"]')[0]
						...
						if (resultado == 'Institucion') {
							...
							if (estado == 'BAJA') {
								panel
										.query('button[action="modificarInstitucion"]')[0]
										.setDisabled(true);
								panel
										.query('button[action="ubicacionesInstitucion"]')[0]
										.setDisabled(true);
							} else {
								panel
										.query('button[action="modificarInstitucion"]')[0]
						...
						if (resultado == 'Animal') {
							...
							var estado = record.getData().estado;
							if (estado == 'BAJA') {
								panel.query('button[action="modificarAnimal"]')[0]
										.setDisabled(true);
								panel
										.query('button[action="cambioUbicacionAnimal"]')[0]
										.setDisabled(true);
								panel
										.query('button[action="modificarPesoAnimal"]')[0]
										.setDisabled(true);
							} else {
								panel.query('button[action="modificarAnimal"]')[0]
										.setDisabled(false);
					...
					busquedaGeneral : function(button) {
						...
						button.up('panelPrincipal2').query(
								'panel[name="operacionesPersona"]')[0].hide();
						button.up('panelPrincipal2').query(
								'panel[name="operacionesInstitucion"]')[0]
								.hide();
						button.up('panelPrincipal2').query(
								'panel[name="operacionesAnimal"]')[0].hide();

						Ext.StoreManager.lookup('ResultadoBusquedaGeneral')
								.load({
					...
					consultaInstitucionesClick : function(button) {
						...
						Ext.StoreManager.lookup(
								'ResultadoBusquedaInstituciones').load({
							scope : this
						});
					...
					consultaPersonasClick : function(button) {
						...
						Ext.StoreManager.lookup('ResultadoBusquedaPersonas')
								.load({
									scope : this
								});
